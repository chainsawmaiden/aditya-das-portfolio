...
export default function NavBar({home, article}: {home?: boolean, article?: boolean}) {
  return (
  <header className={`w-full  top-0 z-50 ${article ? 'px-4 fixed' : 'sticky'} ${home ? 'pb-0' : 'mb-6'} left-0 //bg-white mix-blend-difference text-gray-200 //opacity-50 py-1 font-ppneuebit font-bold text-sm md:text-lg uppercase flex justify-between`}>
    <Link href='/' className="hover:opacity-80 transition ">Work</Link>
    <Link href='/misc' className="hover:opacity-80 transition">Misc</Link>
