  ...
  return (
    <section className='flex flex-col gap-3 text-ppneuemontreal md:text-smnm text-xsnm'>
      ...
      {homeProjects.map((homeProject: homeProjectsType, i: number) => (
        <Link href={homeProject.slug} className={`group hover:opacity-80 transition-all flex flex-col w-full /lg:w-3/4 mx-auto ${i<= -1 && 'md:col-span-2'}`} key={homeProject.name}>
