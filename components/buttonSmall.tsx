...
export default function ButtonSmall({href, text, centered, back}: {href: string, text: string, centered?: boolean, back?: boolean}) {
  return (
    <Link href={href} className={`font-ppmondwest text-mdmw py-3 ${back ? 'px-6' : 'px-9'} bg-gray-400 text-white rounded-full max-w-full min-w-min w-auto hover:bg-gray-600 transition text-center ${centered && 'mx-auto'}`}>
